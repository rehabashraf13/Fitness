...
def get_model_bundle():
    if not os.path.exists("fitness_model_bundle.pkl"):
        raise FileNotFoundError(
            "Model file not found. Please run: python train_model.py first"
        )

    bundle = joblib.load("fitness_model_bundle.pkl")
    return bundle
    ...
    # Fit final model on all data for prediction داخل التطبيق
    model.fit(X, y)
...
def predict_local(payload: dict) -> dict:
    ...
    model = bundle["model"]
    importance_df = bundle["importance_df"]
    ...
    return {
        ...
        "cv_accuracy_mean": round(bundle["cv_accuracy_mean"], 4),
        "cv_accuracy_std": round(bundle["cv_accuracy_std"], 4),
        "cv_f1_mean": round(bundle["cv_f1_mean"], 4),
        "cv_f1_std": round(bundle["cv_f1_std"], 4),
        "cv_scores_each_fold": [round(float(x), 4) for x in bundle["cv_accuracy_scores"]]
    }
...
if submitted:
    ...
    try:
        ...
        r2.metric("5-Fold CV Accuracy", result["cv_accuracy_mean"])
        r3.metric("5-Fold CV Macro F1", result["cv_f1_mean"])
        r4.metric("BMI", bmi)
        ...
        st.markdown("### 📊 5-Fold Cross Validation Details")
        st.write(f"Fold Accuracy Scores: {result['cv_scores_each_fold']}")
        st.write(f"Mean CV Accuracy: {result['cv_accuracy_mean']}")
        st.write(f"CV Accuracy Std: {result['cv_accuracy_std']}")
        st.write(f"Mean CV Macro F1: {result['cv_f1_mean']}")
        st.write(f"CV Macro F1 Std: {result['cv_f1_std']}")
